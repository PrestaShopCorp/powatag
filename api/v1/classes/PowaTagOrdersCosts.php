...
class PowaTagOrdersCosts extends PowaTagAbstract
{
    ...
    public function __construct(stdClass $datas)
    {
        ...
        if (isset($order->paymentCard)) {
            $this->datas->customer->billingAddress = $order->paymentCard->billingAddress;
        }

        if (!isset($this->datas->customer->shippingAddress)) {
            $this->addError(sprintf($this->module->l('Missing shipping address')), PowaTagErrorType::$OTHER_ADDRESS_ERROR);

            return;
        }
        if (!isset($this->datas->customer->shippingAddress->city)) {
            $this->addError(sprintf($this->module->l('City is missing')), PowaTagErrorType::$OTHER_ADDRESS_ERROR);

            return;
        }
        if (empty($order->orderLineItems)) {
            // return SKU_OUT_OF_STOCK as user probably asked for something that became out of stock
            $this->addError(sprintf($this->module->l('order is empty, probably because product became out of stock')), PowaTagErrorType::$SKU_OUT_OF_STOCK);
        ...
        if (isset($this->datas->vouchers)) {
            ...
            if ($vouchers && count($vouchers)) {
                foreach ($vouchers as $voucher) {
                    ...
                    if ($cr->checkValidity($this->context, false, true)) {
                        continue;
                    }
